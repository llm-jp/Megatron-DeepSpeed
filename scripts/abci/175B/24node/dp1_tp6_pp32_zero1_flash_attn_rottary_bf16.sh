...
mp_size=6 # tensor model parallel size
...
checkpoint_path="/groups/gaf51217/fujii/checkpoints/megatron-deepspeed/175B/${jobname}-flash-attn-rope_bf16"
...
mpirun -np $num_gpus \
  ...
  ${megatron_options} \
  --use-mpi \
  --wandb-name "175B-rope-flash-attn-bf16-${jobname}" \
  ${data_options} \
  ${deepspeed_options}
